...
def get_data_info(folder_list, seq_len_range, overlap, sample_times=1, pad_y=False, shuffle=False, sort=True):
    ...
    for folder in folder_list:
        start_t = time.time()
        # Load pose and image sequences
        poses = load_npy('{}{}.npy'.format(par.pose_location, folder)) # (n_images, 6)
        fpaths = []
        for f in list_contents(par.sequence_key+folder+"/image_2"):
            fpaths.append(f['Key'])
        fpaths.sort()
        # Fixed seq_len
...
class ImageSequenceDataset(Dataset):
    ...
    def getGoodPose(self, index):
        if len(self.groundtruth_arr[index]) == 0:
            return self.getGoodPose(index-1)
        else:
            return self.groundtruth_arr[index]

    def __getitem__(self, index):
        #raw_groundtruth = np.hsplit(self.getGoodPose(index), np.array([6]))
        raw_groundtruth = np.hsplit(self.groundtruth_arr[index], np.array([6]))
        groundtruth_sequence = raw_groundtruth[0]
        
        # print(len(groundtruth_sequence))
